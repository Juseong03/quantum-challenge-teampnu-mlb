...
class PKPDDataAugmentation:
    ...
    def _apply_pd_response_augmentation(self, x: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """PD response specific augmentation (입력 x만 변형, y는 안정적으로 유지)"""
        x_aug = x.clone()

        # 1. 시간 축 jitter (time shift)
        if x.size(1) > 0:
            time_shift = torch.randn(x.size(0), 1).to(x.device) * self.time_shift_ratio
        ...
        # 5. y는 원본 유지 (option: 아주 작은 noise만 추가 가능)
        y_aug = y.clone()
        y_aug = y_aug + 0.01 * torch.randn_like(y_aug)  # 안정성 위해 아주 작은 노이즈
