...
class PKPDDataAugmentation:
    """PK/PD Data Augmentation with enhanced techniques"""
    ...
    def _apply_pd_response_augmentation(self, x: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """PD response specific augmentation (apply only to x, keep y stable except optional small noise)"""
        # 1. 입력에만 augmentation
        x_aug, _ = self._apply_gaussian_noise(x, y)        # y는 무시
        x_aug, _ = self._apply_feature_dropout(x_aug, y)   # y는 무시
        x_aug, _ = self._apply_time_warp(x_aug, y)         # y는 무시

        # 2. 타겟은 원본 유지 (원하는 경우 작은 노이즈만 허용)
        y_aug = y.clone()
        # Optional: 작은 가우시안 노이즈 추가 가능
        y_aug = y_aug + 0.01 * torch.randn_like(y_aug)
