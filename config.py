...
class Config:
    ...
    loss_type_pk: str = "mse"        # PK는 일반적으로 mse/mae 정도만
    quantile_q: float = 0.3            # quantile loss 파라미터
    over_weight: float = 2.0           # asymmetric loss 파라미터
...
def parse_args() -> Config:
    ...
    config = Config(
        ...
        loss_type_pk=args.loss_type_pk,        # PK는 일반적으로 mse/mae 정도만
        quantile_q=args.quantile_q,             # quantile loss 파라미터
        hybrid_lambda=args.hybrid_lambda,       # MSE : Quantile 가중치 비율 (0.7 → MSE 70%, Quantile 30%)
        over_weight=args.over_weight,            # asymmetric loss 파라미터
