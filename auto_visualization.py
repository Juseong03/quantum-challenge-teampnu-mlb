...
class AutoVisualizer:
    ...
    def _plot_pd_by_id_time_ver2(self, pd_results, viz_dir, threshold=3.3):
        ...
        for i, subject_id in enumerate(sorted(pd_results['ID'].unique())):
            ...
            ax = axes[i]
            l1, = ax.plot(subject_data['TIME'], subject_data['DV'], 'o-', label='Actual', linewidth=2, markersize=6)
            l2, = ax.plot(subject_data['TIME'], subject_data['PRED'], 's--', label='Predicted', linewidth=2, markersize=6)

            # threshold line
            l3 = ax.axhline(y=threshold, color='red', linestyle='--', linewidth=1, alpha=0.7, label=f'Threshold={threshold}')

            # classification marker
            if 'PRED_CLF' in subject_data.columns:
                clf_pos = subject_data[subject_data['PRED_CLF'] == 1]
                if not clf_pos.empty:
                    ax.scatter(clf_pos['TIME'], clf_pos['PRED'], color='red', s=100, marker='x', label='Pred Clf=1')
